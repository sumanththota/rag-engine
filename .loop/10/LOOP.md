...
- **PRE-FLIGHT GAP (orchestrator check 2026-09-19, read before starting):** no Google OAuth config exists anywhere.
  ...
  - **HAZARD 5 — register the client WITH the `openid` scope, and build the mock around a real `id_token` (verifier round 2, authlib 1.8.0 source-verified; my guidance omitted the scope):** `oauth.register(..., client_kwargs={"scope": "openid email profile", "code_challenge_method": "S256"})`. Authlib only generates a `nonce` when the scope contains `openid`, and `authorize_access_token` (starlette_client/apps.py ~L136) only sets `token["userinfo"]` when `"id_token" in token and "nonce" in state_data`. With no scope, the real Google callback returns a token with no `userinfo` and every sign-in 400s while a mock that hands over a pre-made `userinfo` stays green. Tests must therefore: (a) have the patched `fetch_access_token` return a token that CONTAINS an `id_token` key, so authlib's own internal `parse_id_token(token, nonce=...)` fires; (b) patch `parse_id_token` (AsyncMock) to return the claims, and let `token["userinfo"]` arrive via that path — never inject `userinfo` into the fetched token; (c) in the state-rejection tests ALSO patch `fetch_access_token` and assert it was NOT called (`assert_not_called`) — otherwise removing the state check makes them hit Google's live token endpoint over the network and still see a 4xx; (d) assert the login redirect URL carries `scope` containing `openid`. Drop the bare `except Exception` around `authorize_access_token`: catch `OAuthError` only.
  ...
  - **HAZARD 4 — do NOT call `parse_id_token` yourself (orchestrator guidance error, corrected 2026-09-19 after verifier round 1):** authlib 1.8.0's `parse_id_token(token, nonce, ...)` REQUIRES `nonce`; calling `await oauth.google.parse_id_token(token)` raises TypeError against real Google, which a bare `except Exception` disguises as an auth failure while an AsyncMock hides it entirely. HAZARD 2's "patch `parse_id_token`" means: patch it so authlib's OWN internal call (made by `authorize_access_token`, with the right nonce) returns claims. The route calls only `authorize_access_token(request)` and reads the claims from `token["userinfo"]`. Never catch bare `Exception` around it — catch `OAuthError`. The guard test (CORRECTED in round 2 — the earlier "raising side_effect" idea is wrong, because on the real path authlib itself calls `parse_id_token`): with `parse_id_token` patched as an AsyncMock returning the claims, assert it was awaited EXACTLY ONCE and that the call carried a non-empty `nonce`. A route that also calls it directly makes the count 2 and fails the test.
